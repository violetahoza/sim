...
class ExperimentMetrics:
    ...
    def to_dict(self) -> dict:
        ...
        cloud_msgs_unique_accepted = max(0, self.events_reflected_in_cloud - self.duplicate_deliveries)
        e2e_link_dr = round(self.sensor_to_edge_delivery_ratio * self.backhaul_delivery_ratio, 4)

        d: dict = {
            "scenario_name": self.scenario_name, "protocol": self.protocol, "architecture": self.architecture,
            "traffic_level": self.traffic_level, "num_spots": self.num_spots, "sim_duration_s": self.sim_duration_s,

            "events_generated": self.events_generated, "valid_state_changes": self.valid_state_changes,
            "initial_snapshots_generated": self.initial_snapshots_generated, "heartbeats_generated": self.heartbeats_generated,
            "duplicate_sends_generated": self.duplicate_sends_generated, "heartbeat_interval_s": self.heartbeat_interval_s,

            "sensor_to_edge_msgs": self.sensor_to_edge_msgs, "sensor_link_dropped": self.sensor_link_dropped, "wire_frames_delivered": wire_frames_delivered,
            "sensor_to_edge_delivery_ratio": self.sensor_to_edge_delivery_ratio, "sensor_to_edge_bytes": self.sensor_to_edge_bytes,

            "protocol_bytes": self.protocol_bytes, "retransmissions_total": self.retransmissions_total,
            "duplicate_deliveries": self.duplicate_deliveries, "broker_overhead_score": self.broker_overhead_score,

            "events_reflected_in_cloud": self.events_reflected_in_cloud, "cloud_msgs_unique_accepted": cloud_msgs_unique_accepted,
            "cloud_duplicates_dropped": self.duplicate_deliveries, "cloud_reflection_ratio": self.cloud_reflection_ratio,


            "physical_delivery_ratio": self.physical_delivery_ratio, "e2e_link_delivery_ratio": e2e_link_dr,

            "fault_injected_count": self.fault_injected_count, "warmup_excluded_samples": self.warmup_excluded_samples,

            "latency_mean_ms": self.latency_mean_ms, "latency_p50_ms": self.latency_p50_ms, "latency_p95_ms": self.latency_p95_ms,
            "latency_p99_ms": self.latency_p99_ms, "latency_min_ms": self.latency_min_ms, "latency_max_ms": self.latency_max_ms
        }

        if is_cloud_only:
            d["cloud_msgs_received"] = self.events_reflected_in_cloud
            ...
            d["e2e_link_delivery_ratio"] = e2e_link_dr
