...
from simulator.constants import (
    COAP_HEADER_BYTES, COAP_TOKEN_BYTES, COAP_PAYLOAD_MARKER,
    COAP_URI_PATH_OPTION_EST, COAP_ACK_BYTES, UDP_TRANSPORT_OVERHEAD,
)
...
class SimulatedCoAPBackend(ProtocolBackend):
    ...
    def __init__(self, config: CoAPConfig, clock: SimClock, subscriber_cb: CloudRecvCallback,
                 loss_rate: float = 0.02, seed: int = 0,
                 ack_one_way_delay_s: float = 0.030, ack_jitter_s: float = 0.010,
                 downlink_loss_rate: Optional[float] = None,
                 loss_provider: Optional[Callable[[float], float]] = None) -> None:
        self.config = config
        self.clock = clock
