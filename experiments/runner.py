...
class ExperimentRunner:
    ...
    async def _run_simulated(self) -> ExperimentMetrics:
        ...
        def des_progress(virtual_now: float, end_time: float) -> None:
            ...
            es = edge.summary()
            snap = {
                "elapsed_s": round(time.time() - self._start_time, 1), "wall_duration_s": round(end_time / cfg.traffic.time_scale, 1),
                "simulated_elapsed_s": round(virtual_now, 0), "simulated_duration_s": end_time,
                ...
                "cloud_events": cloud.received_events, "occupancy": sensors.occupancy_snapshot(), "spot_states": dict(self._spot_states), "edge": es,
            }
    ...
    def _collect_metrics_simulated(self, cfg, sensors, link, edge: EdgeNode, cloud: CloudBackend, backhaul_link, protocol_bytes: int = 0, retransmits: int = 0, dup_deliveries: int = 0) -> ExperimentMetrics:
        ...
        physical_delivery_ratio = s2e_dr * backhaul_dr
    ...
    async def _collect_metrics_real(self, cfg, sensors, link, edge_proc, cloud_proc, backend, retransmits: int = 0, dup_deliveries: int = 0) -> ExperimentMetrics:
        ...
        if arch == "cloud_only":
            ...
            aggregation_ratio = (e2c_msgs / s2e_received) if s2e_received > 0 else 1.0
            message_reduction_ratio = max(0.0, 1.0 - aggregation_ratio)
            events_per_cloud_message = (events_reflected / e2c_msgs if e2c_msgs > 0 else 0.0) if arch == "edge_aggregated" else 1.0
    ...
    def _log_done(self, cfg, metrics: ExperimentMetrics, cloud_events: int) -> None:
        ...
        if arch == "cloud_only":
            logger.info(
                ...
                f"cloud_reflection={m.cloud_reflection_ratio:.1%}  "
